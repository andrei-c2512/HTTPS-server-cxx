...
namespace thm::core{
    ...
    public:
            SecureConnection(asio::io_context& context0, asio::ssl::context& sslContext,
                    SslSocket stream,
                    util::ds::TsQueue<std::shared_ptr<messageTypeIn>>& queue0)
    ...
    public:
            ...
            void disconnect() override {
                    if (isConnected()) {
                            ...
                            if (!ec) 
                                    util::log::info("SSL shutdown completed");
                            else
                                    util::log::error("Failed to shut down SSL: " + ec.message());

                            ssl_stream.lowest_layer().close(ec);

                            if (!ec)
                                    util::log::info("Succesfully disconnected");
                            else
                                    util::log::error("Failed to disconnect: " + ec.message());
                    ...
                    asio::async_write(ssl_stream, asio::buffer(str.data(), str.size()),
                            [this , str](std::error_code ec, std::size_t length) {
                                    if (!ec) {
                                            ...
                                            util::log::message("Sucessfully wrote a message");
                                            util::log::info("Request size: " + std::to_string(length));
                                            util::log::info(str);
                                            ...
                                            util::log::error("Failed to write a request to the server");
                            });
            }
    protected:
